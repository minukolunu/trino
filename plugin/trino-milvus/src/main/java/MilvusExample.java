...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// Note:
// Due do a technical limitation, the Milvus 2.0 not allow to create multi-vector-fields within a
// collection.
// So this example only create a single vector field in the collection, but we suppose the next
// version
...
public class MilvusExample {
  ...
  private R<RpcStatus> createCollection(long timeoutMilliseconds) {
    ...
    R<RpcStatus> response =
        milvusClient
            .withTimeout(timeoutMilliseconds, TimeUnit.MILLISECONDS)
  ...
  private R<RpcStatus> releaseCollection() {
    ...
    R<RpcStatus> response =
        milvusClient.releaseCollection(
            ReleaseCollectionParam.newBuilder().withCollectionName(COLLECTION_NAME).build());
    handleResponseStatus(response);
    System.out.println(response);
    return response;
  }
  ...
  private R<GetCollectionStatisticsResponse> getCollectionStatistics() {
    ...
    R<GetCollectionStatisticsResponse> response =
        milvusClient.getCollectionStatistics(
            GetCollectionStatisticsParam.newBuilder().withCollectionName(COLLECTION_NAME).build());
    handleResponseStatus(response);
  ...
  private R<ShowPartitionsResponse> showPartitions() {
    ...
    R<ShowPartitionsResponse> response =
        milvusClient.showPartitions(
            ShowPartitionsParam.newBuilder().withCollectionName(COLLECTION_NAME).build());
    handleResponseStatus(response);
    System.out.println(response);
    return response;
  }
  ...
  private R<GetIndexBuildProgressResponse> getIndexBuildProgress() {
    ...
    R<GetIndexBuildProgressResponse> response =
  ...
  //
  //        handleResponseStatus(response);
  //        SearchResultsWrapper wrapper = new
  // SearchResultsWrapper(response.getData().getResults());
  //        for (int i = 0; i < vectors.size(); ++i) {
  //            System.out.println("Search result of No." + i);
  //            List<SearchResultsWrapper.IDScore> scores = wrapper.getIDScore(i);
  //            System.out.println(scores);
  ...
  private R<QueryResults> query(String expr) {
    ...
    System.out.println(
        ID_FIELD + ":" + wrapper.getFieldWrapper(ID_FIELD).getFieldData().toString());
    System.out.println(
        AGE_FIELD + ":" + wrapper.getFieldWrapper(AGE_FIELD).getFieldData().toString());
  ...
  private R<ManualCompactionResponse> compact() {
    ...
    R<ManualCompactionResponse> response =
        milvusClient.manualCompact(
            ManualCompactParam.newBuilder().withCollectionName(COLLECTION_NAME).build());
    handleResponseStatus(response);
    return response;
  }
  ...
  public static void main(String[] args) {
    ...
    //        example.releasePartition(partitionName); // releasing partitions after loading
    // collection is not supported currently
    example.releaseCollection();
